...
Controllers.renderAdminPage = async function (req, res/* , next */) {
	...
	if (start && end) {
		...
		if (req.query.type === 'firstpost') {
			let firstPosters = [];
			const uidsWhoMadePostsInTimerange = Object.keys(uids);
			await Promise.all(uidsWhoMadePostsInTimerange.map(async (uid) => {
				const firstPid = await db.getSortedSetRange(`uid:${uid}:posts`, 0, 0);
				if (uids[uid].includes(parseInt(firstPid, 10))) {
					firstPosters.push(uid);
				}
			}));
			users = await loadUserInfo(req.uid, firstPosters);
		} else {
...
async function loadUserInfo(callerUid, uids) {
	...
	async function getFirstPostTime() {
		return Promise.all(uids.map(async (uid) => {
			const firstPid = await db.getSortedSetRange(`uid:${uid}:posts`, 0, 0);
			const timestamp = await db.getObjectField(`post:${firstPid}`, 'timestamp');
			return timestamp;
		}));
	}
	...
	const [isAdmin, userData, lastonline, confirmObjs, ips, firstPostTime] = await Promise.all([
	...
	userData.forEach((user, index) => {
		if (user) {
			...
			user.firstpost = firstPostTime[index];
			user.firstpostISO = utils.toISOString(user.firstpost);
			user.ips = ips[index];
